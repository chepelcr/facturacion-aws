...
function selectTipoCambio() {
    ...
    var option = $('#' + factura_activa).find('.currencyCode').find('option:selected');

    //Obtener el valor data-currencyCode del option
    var moneda = $(option).data('currencycode');

    if (moneda == 'CRC') {
        $('#' + factura_activa).find('.exchangeRate').val(1);
        activar_campo_clase('exchangeRate', true, factura_activa);
    } else if (moneda == 'USD') {
        $('#' + factura_activa).find('.exchangeRate').val(cambio_venta);
        activar_campo_clase('exchangeRate', true, factura_activa);
    } else {
        $('#' + factura_activa).find('.exchangeRate').val('');
...
function ver_factura(id_factura) {
    ...
    if ($('#' + factura).length == 0 || $('#' + factura).is(':empty')) {
        ...
        var documentType = $('#' + factura_activa).find('.documentTypeId').find('option:selected');
        ...
        $('.btn-factura').removeClass('btn-success btn-warning btn-danger btn-secondary btn-purple btn-info').addClass('btn-dark');
