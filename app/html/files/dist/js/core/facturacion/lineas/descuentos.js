...
function agregar_descuento(boton = null, linea = null) {
  ...
  var cantidad_lineas = $(linea)
    .find(".discounts")
    .find(".discountLine").length;

  //Si ya hay 5 lineas de descuento, mostrar un mensaje de error
  if (cantidad_lineas >= 5) {
    notificacion(
      "Solo se pueden agregar 5 descuentos por linea de detalle.",
      "",
      "warning"
    );
    return;
  ...
  if (ultima_linea.find(".discount_amount").val() == 0) {
    notificacion(
      "No se puede agregar un descuento si el anterior es 0.",
      "",
      "warning"
    );
    return;
...
function eliminarDescuentosLinea(linea) {
  ...
  limpiar_linea_descuento(
    linea.find(".discounts").find(".discountLine").first()
  );
}
...
function calcular_descuento(discountLine = null) {
  ...
  if (discountLine != null) {
    ...
    $(linea_activa)
      .find(".total_discount_money")
      .val(formato_moneda(total_descuento, 2, monedaDocumento));
  } //Fin de validacion de linea
