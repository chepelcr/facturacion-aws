...
/**
 * Validar los descuentos de una linea de detalle
 * @param {*} linea Linea de detalle
 * @returns Indica si la linea de detalle es valida
 */
function validar_descuentos_detalle(linea) {
    //Obtener la tabla de descuentos
    const discountsTable = linea.find(".discounts");
    ...
    discountLines.each(function (index, discountLine) {
        ...
        if (discountReason == "" && (discountPercentage == "" || discountPercentage == 0)) {
            ...
            if (discountLines.length == 1) {
                ...
                //Colocar el borde rojo en el porcentaje
                if (discountPercentage == "" || discountPercentage == 0) {
                    $(discountLine).find(".discount_percentage").addClass("border-danger");
                }

                //Colocar el borde rojo en la razon
                if (discountReason == "") {
            ...
            if (discountReason != "" && discountPercentage > 0) {
                $(discountLine).find(".discount_reason").removeClass("border-danger");
                $(discountLine).find(".discount_percentage").removeClass("border-danger");
            } else {
                valid = false;
                ...
                //Si la razon no existe, poner el borde rojo en la razon
                if (discountReason == "") {
                    $(discountLine).find(".discount_reason").addClass("border-danger");
                } else {
                    $(discountLine).find(".discount_reason").removeClass("border-danger");
...
function agregar_descuentos(linea_detalle = null, descuentos = []) {
    if (linea_detalle != null) {
        ...
        validar_descuentos_detalle(linea_detalle);
    }
}
...
function calcular_descuentos(linea_detalle = null) {
    ...
    if (linea_detalle != null) {
        ...
        $(linea_detalle).find(".total_discount_money").val(formato_moneda(descuento_total, 2, monedaDocumento));
...
function calcular_descuento(discountLine = null, neto = 0, linea_activa = null) {
    ...
    if (discountLine != null) {
        ...
        $(linea_activa).find(".total_discount_money").val(formato_moneda(total_descuento, 2, monedaDocumento));
    } //Fin de validacion de linea
