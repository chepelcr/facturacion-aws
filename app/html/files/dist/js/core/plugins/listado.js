...
function getLoadPage() {
    let html = "<div class='d-flex justify-content-center'><div class='spinner-border text-primary' role='status'><span class='sr-only'>Cargando...</span></div></div>";
...
function getErrorPage(error) {
    let html = "<div class='card card-danger'><div class='card-header'><h3 class='card-title" +
        "'>Error</h3></div><div class='card-body'><p>" + error.error + "</p></div></div>";
...
function cargar_contenido(contenido, estado = 'cargando') {
    ...
    if (submodulo != '' && submodulo != 'auditorias' && submodulo != 'errores' && submodulo != 'ordenes' && submodulo != 'documentos') {
...
function filtrar_elemento(contenedor, claseElemento, filtro) {
    if (filtro != '') {
        ...
        $("#" + contenedor + " ." + claseElemento).each(function (index, value) {
            ...
            if ($(this).text().toLowerCase().indexOf(filtro) > -1 || $(this).find('input').val().toLowerCase().indexOf(filtro) > -1) {
                $(this).show();
            } else {
                $(this).hide();
            }
