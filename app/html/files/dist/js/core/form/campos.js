...
function vaciar_campos(elemento = "") {
    ...
    if (modulo_activo == "empresa" && submodulo_activo == "productos") {
        ...
        //Colocar 0 en los campos inp-product-money
        element.find(".inp-product-money").val(formato_moneda(0, 2, monedaDocumento));
    }

    //Eliminar el borde rojo de los campos que tienen required como atributo
...
function desactivar_permisos(elemento = "") {
    if (elemento != "") {
        ...
        element.find(".btn-permiso").addClass("btn-secondary");

        element.find(".inp-chk").prop("checked", false);
        element.find(".inp-chk").val("");
    } else {
        //Eliminar la clase btn-info, btn-warning, btn-danger de los btn-permiso
        $(".btn-permiso")
            .removeClass("btn-info")
