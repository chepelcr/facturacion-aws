            ...
            case 'empresa':
                switch (submodulo) {
                    case 'productos':
                        ...
                        $('#' + form_activo).find('.salePrice').val(0);
                        $('#' + form_activo).find('.taxValue').val(0);
                        $('#' + form_activo).find('.unitPrice').val(0);

                        //Enfocar el campo de descripcion
                        $('#' + form_activo).find('.description').focus();
                    ...
                    case 'clientes':
                        ...
                        $('#' + form_activo).find('.identification_number').focus();
                        break;
                }
                break;
                ...
                if (!response.error) {
                    Swal.fire({
                        ...
                        if (nombre_elemento != '') {
                            //Esperar un segundo
                            setTimeout(function () {
                                cargar_listado(modulo_activo, submodulo_activo, nombre_vista_modulo, nombre_vista_submodulo, base + modulo_activo + "/" + submodulo_activo + "/listado");
                            }, 1000);
                        }//Fin de if
                    })//Fin del mensaje
                }
...
function llenarObjeto(nombre_form, objeto, estado) {
        ...
        if (key == 'identification') {
            ...
            $.each(valor, function (i, code) {
                ...
                if (code.typeCode == '01') {
                    ...
                    $('#' + nombre_form).find(".saleCode").val(code.codeNumber);
                    ...
                    $('#' + nombre_form).find(".internalCode").val(code.codeNumber);
                ...
                if (valor != null) {
                    $.each(valor, function (inner_key, inner_value) {
                        ...
                        if (inner_key == 'category_productType') {
                            productTypes = $('#' + nombre_form).find('.category_productType_id option');
                            ...
                            $('#' + nombre_form).find("." + inner_key).val(inner_value);
                        }
                    });
                }
            } else {
    ...
    if (modulo_activo == 'empresa' && submodulo_activo == 'clientes') {
        ...
        calcular_valor_producto(nombre_form);
        campos_cabys(estado, nombre_form);
    }

    //Abrir el card
    $('#' + nombre_form).find('.card').CardWidget('expand');
}
