...
function agregar_linea_activa(producto, cantidad = 1, precio_final = 0) {
    ...
    if ($("#" + factura_activa).find(".detail").length == 1) {
        ...
        console.log(linea.find(".saleCode").val());
        //console.log(linea.find(".cabys").val());

        if ((linea.find(".saleCode").val() != 0 || linea.find(".saleCode").val() != '')) { //&& (linea.find(".cabys").val() != 0 || linea.find(".cabys").val() != '')) {
            console.log('Linea encontrada');

            console.log('Validando linea');

            //console.log('Codigo: ' + linea.find(".cabys").val());
            console.log('Codigo venta: ' + linea.find(".saleCode").val());

            linea = aumentar_linea();
        }

        /*//Si la linea de la factura no tiene gnl y codigo
        if ($("#" + factura_activa).find(".detail").find(".saleCode").val() == '' && $("#" + factura_activa).find(".detail").find(".cabys").val() == '') {
            linea = $("#" + factura_activa).find(".detail");
        } else {
            linea = aumentar_linea();
        }*/
        ...
        $('#' + factura_activa).find('.detail').each(function () {
            console.log('Validando linea');

            //Si la linea es igual al producto
            if (/*$(this).find(".cabys").val() == codigo && */$(this).find(".saleCode").val() == codigo_venta) {
    ...
    if (producto.discounts != null && producto.discounts.length > 0) {
        console.log('Aca se van a agregar los descuentos');
    ...
    if (producto.taxes != null && producto.taxes.length > 0) {
        console.log('Aca se van a agregar los impuestos');
    ...
    if (precio_final != 0 && precio_final != salePrice) {
        ...
        linea.find(".salePrice").val(precio_final);

        //Colocar el precio en salePrice_money
        linea.find(".salePrice_money").val(formato_moneda(precio_final, 2));
    } else {
        console.log('Precio normal: ' + salePrice);
        ...
        linea.find(".salePrice_money").val(formato_moneda(salePrice, 2));
    }
...
function aumentar_linea() {
    ...
    linea = $("#" + factura_activa).find(".detail").last();
...
function contar_lineas() {
    ...
    $('#' + factura_activa).find('.detail').each(function () {
        ...
        console.log('Recorriendo inputs:' + inputs.length);
        ...
        inputs.each(function (index, input) {
            ...
            if ($(input).attr("name") != undefined) {
                ...
                //Reemplazar el 'details[x]['taxes'][x]' por 'details[x]['taxes'][lineNumber]' ("['taxes'][" + lineNumber + "]")
                //name = name.replace(/\['details'\]\[d+\]/g, "['details'][" + () + "]");

                console.log("Nuevo nombre: " + newFieldName);
...
function cloneLine() {
    ...
    $('#' + factura_activa).find('.cont-details').append(
        $('#' + factura_activa).find(".detail").last().clone());
...
function totales() {
    ...
    $('#' + factura_activa).find(".detail").each(function (i, item) {
    ...
    neto = formato_moneda(neto, 2);
    descuentos = formato_moneda(descuentos, 2);
    subtotal = formato_moneda(subtotal, 2);
