...
function crearBotonFactura(numero_documento) {
    boton = '<div class="col-md-4 col-lg col-md col-sm p-1 col-btn-fct-' + numero_documento + '">' +
        '<button class="btn btn-factura nav-button btn-block" type="button" onclick="ver_factura(' + numero_documento + ')" id="btn_factura_' + numero_documento + '" data-toggle="tooltip" data-placement="top" title="Documento ' + numero_documento + '" ' +
        'value="' + numero_documento + '">Documento ' + numero_documento + '</button></div>';
        ...
        Pace.track(function () {
            $.ajax({
                ...
                error: function (jqXHR, textStatus, errorThrown) {
                    ...
                    mensajeAutomatico('Error', json.error, 'error');

                    errorPage = getErrorPage(json);

                    $('#listado_documentos').empty().append(errorPage);
...
$(document).ready(function () {
    ...
    $(document).on('change', '.chk-pg', function () {
        ...
        tiposPago = $('#' + factura_activa).find('.tipo-pago');
        ...
        tiposPago.each(function (i, tipoPago) {
            ...
            if (typeCode == code) {
                ...
                if ($(check).is(':checked')) {
                    //$(tipoPago).prop('checked', true);

                    console.log('Activar ' + typeCode);
                    ...
                    console.log('Desactivar ' + typeCode);
